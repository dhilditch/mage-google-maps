...
Automatically display Google Maps with multiple location markers in your category/archive or single marker maps on single pages via addresses from posts or custom post types. 
...
= Displaying a Map - Basic =
Display your Google Map anywhere using the [map] shortcode and choose a default address if not address is found in content from the plugin options, or simply type one using address, for example: [map address="Mountain View, CA"].

= Displaying a Map - Automatically =
- To activate Automatic Address Quesies, you first have to choose which post types you want to display maps on under Auto Display->Attach To Post Types.
- After that, the post types' available custom fields will be selectable from the plugin options, below the post type choices. More help can be found from the plugins help page.
